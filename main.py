...
def main(arguments: argparse.Namespace):
    ...
    for driver_version in arguments.versions:
        ...
        for test in arguments.tests:
            logging.info(
                "=== RUST DRIVER VERSION %s. TEST: %s ===", driver_version, test
            )
            runner = Run(
                rust_driver_git=arguments.rust_driver_git,
                tag=driver_version,
                test=test,
            ...
            try:
                ...
                if not report:
                    raise EmptyTestResult(
                        f"No result for test '{test}' and driver version {driver_version}"
                    )

                logging.info(
                    "=== RUST DRIVER MATRIX RESULTS FOR DRIVER VERSION %s ===",
                    driver_version,
                )
                logging.info(
                    "\n".join(
                ...
                results[driver_version][test]["time"] = str(
                    timedelta(
                        seconds=results[driver_version][test]["testsuite_summary"][
                            "time"
                        ]
                    )
                )[:-3]
                ...
                failure_reason = traceback.format_exception(
    ...
    if arguments.recipients:
        ...
        email_report["driver_remote"] = get_driver_origin_remote(
            arguments.rust_driver_git
        )
...
def extract_n_latest_repo_tags(
    repo_directory: str, latest_tags_size: int = 2
) -> List[str]:
    ...
    try:
        subprocess.check_output(
            "git checkout .", shell=True, stderr=subprocess.STDOUT, cwd=repo_directory
        )
        lines = (
            subprocess.check_output(
                "git tag --sort=-creatordate | grep '^v[0-9]*\\.[0-9]*\\.[0-9]*$'",
                shell=True,
...
def get_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "rust_driver_git", help="folder with git repository of rust-driver"
    )
    parser.add_argument(
        "--versions",
        default=versions,
    ...
    parser.add_argument(
        "--scylla-version",
        help="relocatable scylla version to use",
        default=os.environ.get("SCYLLA_VERSION", None),
    )
    parser.add_argument(
        "--rust-driver-versions-size",
        help="The number of the latest versions that will test."
    ...
    parser.add_argument(
        "--recipients",
        help="whom to send mail at the end of the run",
        nargs="+",
        default=None,
    )
    parser.add_argument(
    ...
    if arguments.rust_driver_versions_size:
        arguments.versions = extract_n_latest_repo_tags(
            repo_directory=arguments.rust_driver_git,
