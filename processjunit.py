...
class ProcessJUnit:
    ...
    def _create_report(self):
        def get_attribute():
            return (
                literal_eval(testsuite_element.attrib[key].replace(",", ""))
                if key in testsuite_element.attrib
                else 0
            )
        ...
        for testsuite_element in tree.iter("testsuite"):
            ...
            for key in testcase_keys:
                testcase_keys[key] = (
                    get_attribute()
                    if key != "ignored_on_failure"
                    else testcase_keys[key]
                )
        ...
        new_tree.attrib.update(
            {key: str(value) for key, value in self._summary.items()}
        )
        new_tree.attrib["time"] = f"{testsuite_summary_keys['time']}:.3f"
        logging.info(
            "Creating a new report file in '%s' path", self.summary_report_path
        )
        self.summary_report_path.parent.mkdir(exist_ok=True)
        with self.summary_report_path.open(mode="w", encoding="utf-8") as file:
            file.write(
                ElementTree.tostring(element=new_tree, encoding="utf-8").decode()
            )
    ...
    def save_after_analysis(self) -> None:
        ...
        """
        original_test_result_xml = Path(
            self.tests_result_xml.parent
        ) / self.tests_result_xml.name.replace(".xml", "_origin.xml")
        ...
        for testsuite_element in tree.iter("testsuite"):
            testsuit_child = ElementTree.SubElement(
                new_tree, "testsuite", attrib=testsuite_element.attrib
            )
            for element in testsuite_element.iter("testcase"):
                testcase_element = ElementTree.SubElement(
                    testsuit_child, "testcase", attrib=element.attrib
                )
                if len(list(element.iter())) == 2:
                    element_test_details = list(element.iter())[1]
                    tag_name = element_test_details.tag
                    if (
                        element_test_details.tag == "failure"
                        and element_test_details.attrib["message"].replace(
                            "failed ", ""
                        )
                        in self.ignore_set
                    ):
                        logging.info(
                            "Failed test is ignored for %s driver version. Failure message: %s",
                            self.tag,
                            element_test_details.text,
                        )
                        ...
                        testsuit_child.attrib["failures"] = str(
                            int(testsuit_child.attrib["failures"]) - 1
                        )
        ...
        with self.tests_result_xml.open(mode="w", encoding="utf-8") as file:
            file.write(
                minidom.parseString(
                    ElementTree.tostring(element=new_tree, encoding="utf-8")
                ).toprettyxml(indent="  ")
            )
    ...
    def is_failed(self) -> bool:
        return not (
            sum(
                [
                    test_info["errors"] + test_info["failures"]
                    for test_info in self.summary.values()
                ]
            )
            == 0
        )
